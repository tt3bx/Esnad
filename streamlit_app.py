...
# ---------------------------------------------------------
    ...
    st.markdown("تحليل أداء الرحلات السابقة وإحصائيات التسليم في القطاع الجنوبي (جازان)")
    st.markdown("---")
    ...
    # بيانات الجدول (تم تحديثها لمناطق جازان الحدودية)
    data = {
        "التوقيت": ["2026-05-02 14:30", "2026-05-02 16:15", "2026-05-02 18:00"],
        "المنطقة": ["قطاع الخوبة", "منفذ الطوال", "جبل دخان"],
        "الحمولة": ["إعاشة", "ذخيرة", "إعاشة"],
        "الإحداثيات": ["16.8847, 43.1428", "16.4716, 42.9324", "16.7800, 43.0800"],
    ...
    st.markdown("### 🗺️ مواقع العمليات الأخيرة (الحدود الجنوبية - جازان)")
    
    # إحداثيات الخريطة (نقاط في جازان/الحدود)
    map_data = pd.DataFrame({
        'lat': [16.8847, 16.4716, 16.7800],
        'lon': [43.1428, 42.9324, 43.0800]
    })
    
    # عرض الخريطة مع تخصيص لون النقاط للأحمر (ميزة متوفرة في الإصدارات الحديثة من Streamlit)
    # إذا كان إصدار Streamlit قديماً، سيتم عرض النقاط باللون الافتراضي (وهو أحمر/وردي)
    st.map(map_data, color="#ff0000", size=150)
